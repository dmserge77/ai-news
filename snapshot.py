...
Накопители лежат в data/*.json и теперь хранятся в git — это основной архив.
Снимок нужен как вторая страховка: он одним файлом фиксирует всё состояние
на момент времени. Если правка или сборка что-то испортит, из снимка можно
поднять накопители обратно, не разбираясь с историей коммитов.
...
from news.config import CATEGORIES as CATEGORY_INFO
...
def cat_path(cat):
    return os.path.join(DATA_DIR, cat + ".json")
