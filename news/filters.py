...
# в тексте попадаться могут. Вынесен из функции, чтобы не пересобирать
# список на каждом вызове — журнал отказов зовёт эту проверку часто.
...
JOB_EDU_MARKERS = [
    "курс", "обучение", "школа", "интенсив", "вебинар", "тренинг",
...
def job_reject_reason(title, desc, remote_confirmed=False):
    ...
    # 5. То же блок-слово, но только в описании. Причина названа отдельно —
    #    и это не придирка к формулировке. Замер 18.09.2026 показал, что так
    #    отсеиваются профильные вакансии: у «Аналитика-инженера по машинному
    #    обучению» в требованиях были «водительские права», а у «Главного
    #    специалиста по обучению нейросетей» — слово «обучение». Видно это
    #    только в журнале отказов, и только по такой пометке.
    if any(w in text for w in JOB_BLOCK_PROFESSIONS):
