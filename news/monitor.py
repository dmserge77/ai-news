...
class SourcesLog:
    ...
    def _label(self, rec, at):
        ...
        # Тревоги ещё нет, но и «работает» бывает неправдой: источник ответил
        # и не дал ни одной записи. Пишем как есть — молчание видно сразу,
        # а не через три прогона, когда включится предупреждение.
        if rec.get("status") == EMPTY:
            return "лента пуста"
        if rec.get("status") == FILTERED:
            return "не проходит фильтр"
        return "работает"
